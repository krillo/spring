...
function adrotate_stats_nav($type, $id, $month, $year) {
	...
	if($type == 'global-report') $page = '';
	if($type == 'advertiser') $page = '&view=report&ad='.$id;
	if($type == 'advertiser-global') $type = 'advertiser';
...
function adrotate_prepare_global_report() {
	...
	$stats['lastclicks'] = adrotate_array_unique($wpdb->get_results("SELECT `timer`, `bannerid`, `useragent`, `country`, `city` FROM `".$wpdb->prefix."adrotate_tracker` WHERE `stat` = 'c' AND `ipaddress` != 0 ORDER BY `timer` DESC LIMIT 50;", ARRAY_A));
...
function adrotate_prepare_advertiser_report($user, $ads) {
	...
	if($ads) {
		...
		SELECT `".$prefix."adrotate`.`title`, SUM(`".$prefix."adrotate_stats`.`clicks`) as `clicks` 
		FROM `".$prefix."adrotate`, `".$prefix."adrotate_linkmeta`, `".$prefix."adrotate_stats` 
		WHERE `".$prefix."adrotate`.`id` = `".$prefix."adrotate_linkmeta`.`ad` 
		AND `".$prefix."adrotate_linkmeta`.`ad` = `".$prefix."adrotate_stats`.`ad` 
		AND `".$prefix."adrotate`.`tracker` = 'Y' 
		AND `".$prefix."adrotate`.`type` = 'active' 
		AND `".$prefix."adrotate_linkmeta`.`user` = %d
		GROUP BY `".$prefix."adrotate`.`id`
		ORDER BY `".$prefix."adrotate_stats`.`clicks` DESC LIMIT 1;
		", $user), ARRAY_A);
		...
		AND `".$prefix."adrotate_linkmeta`.`user` = %d
		GROUP BY `".$prefix."adrotate`.`id`
		ORDER BY `".$prefix."adrotate_stats`.`clicks` ASC LIMIT 1;
		", $user), ARRAY_A);
