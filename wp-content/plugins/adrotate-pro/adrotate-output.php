...
function adrotate_ad($banner_id, $individual = true, $group = 0, $block = 0) {
	global $wpdb, $adrotate_config, $adrotate_crawlers, $adrotate_debug, $adrotate_geo;
	...
	if($banner_id) {
		if($individual == true) {
			$banner = $wpdb->get_row($wpdb->prepare("SELECT 
										...
										`link`, `image`, `timeframe`, 
										`timeframelength`, `timeframeclicks`, 
										`timeframeimpressions`, `crate`, 
										`irate`, `cbudget`, `ibudget` 
									FROM 
			...
			if($banner) {
				...
				if($adrotate_config['enable_stats'] == 'Y' AND ($banner->timeframe == 'hour' OR $banner->timeframe == 'day' OR $banner->timeframe == 'week')) {
					$selected = adrotate_filter_timeframe($selected, $banner);
				}
	
				if($banner->crate > 0 AND ($banner->cbudget - $banner->crate <= 0)) {
					$selected = array_diff_key($selected, array($banner->id => 0));
				}
	
				if($banner->irate > 0 AND ($banner->ibudget - $banner->irate <= 0)) {
					$selected = array_diff_key($selected, array($banner->id => 0));
				}
			} else {
				$selected = false;
			...
			if($adrotate_config['enable_stats'] == 'Y') {
				if(($adrotate_config['enable_loggedin_impressions'] == 'Y' AND is_user_logged_in()) OR !is_user_logged_in()) {
					...
					if($ip < $impression_timer AND $nocrawler == true AND (strlen($useragent_trim) > 0 OR !empty($useragent))) {
						...
						$wpdb->insert($wpdb->prefix."adrotate_tracker", array('ipaddress' => $remote_ip, 'timer' => $now, 'bannerid' => $banner_id, 'stat' => 'i', 'useragent' => '', 'country' => $adrotate_geo['country'], 'city' => $adrotate_geo['city']));
					}
				}
			}
		} else {
	...
	if($group_ids) {
		...
		if($group) {
			...
			if($ads) {
				...
				foreach($ads as $ad) {
					$selected[$ad->id] = $ad->weight;
					$selected = adrotate_filter_schedule($selected, $ad);
	
					if($adrotate_config['enable_stats'] == 'Y' AND ($ad->timeframe == 'hour' OR $ad->timeframe == 'day' OR $ad->timeframe == 'week')) {
						$selected = adrotate_filter_timeframe($selected, $ad);
					}
					...
					if($ad->crate > 0 AND ($ad->cbudget - $ad->crate <= 0)) {
						$selected = array_diff_key($selected, array($ad->id => 0));
					}
		
					if($ad->irate > 0 AND ($ad->ibudget - $ad->irate <= 0)) {
						$selected = array_diff_key($selected, array($ad->id => 0));
					}
				}
			...
			if($groups) {
				...
				if($results) {
					...
					foreach($results as $result) {
						...
						if($adrotate_config['enable_stats'] == 'Y' AND ($result->timeframe == 'hour' OR $result->timeframe == 'day' OR $result->timeframe == 'week' OR $result->timeframe == 'month')) {
							$selected = adrotate_filter_timeframe($selected, $result);
						}
	
						if($result->crate > 0 AND ($result->cbudget - $result->crate <= 0)) {
							$selected = array_diff_key($selected, array($result->id => 0));
						}
			
						if($result->irate > 0 AND ($result->ibudget - $result->irate <= 0)) {
							$selected = array_diff_key($selected, array($result->id => 0));
						}
						$i++;
					}
...
function adrotate_custom_head() {
	...
	if($blocks) {
		$output_css .= "<!-- AdRotate CSS (Deprecated) -->\n";
		...
		$output_css .= ".block_left { clear:left; }\n";
		$output_css .= ".block_right { clear:right; }\n";
		$output_css .= ".block_both { clear:both; }\n";
		if($adrotate_config['widgetpadding'] == "Y") { 
			$output_css .= ".widget_adrotate_widgets { overflow:hidden; padding:0; }\n"; 
		}
		$output_css .= "</style>\n";
		$output_css .= "<!-- /AdRotate CSS (Deprecated) -->\n\n";
		unset($blocks);
	...
	$groups = $wpdb->get_results("SELECT `id`, `modus`, `gridrows`, `gridcolumns`, `adwidth`, `adheight`, `admargin`, `admargin_bottom`, `admargin_left`, `admargin_right`, `adspeed` FROM `".$wpdb->prefix."adrotate_groups` WHERE `name` != '' AND `modus` > 0 ORDER BY `id` ASC;");
	if($groups) {
		...
		foreach($groups as $group) {
			if($group->adwidth != 'auto') {
				...
				if($group->modus == 2) {
					$width_sum = $group->admargin_left + $group->adwidth + $group->admargin_right;
					$width_min = $group->admargin_left + $group->admargin_right;
			...
			if($group->adheight != 'auto') {
				...
				if($group->modus == 2) {
					$height_sum = $group->admargin + $group->adheight + $group->admargin_bottom; 
					$height_min = $group->admargin + $group->admargin_bottom;
			...
			if($group->modus == 1) {
				...
				$array_css[] = "\t.g-".$group->id." { ".$dynamic_width." ".$dynamic_height." margin-top:".$group->admargin."px; margin-right:".$group->admargin_right."px; margin-bottom:".$group->admargin_bottom."px; margin-left:".$group->admargin_left."px; }\n";
				$array_css[] = "\t.a-".$group->id." { ".$dynamic_width." ".$dynamic_height." }\n";
			}
	
			if($group->modus == 2) {
				$array_css[] = "\t.b-".$group->id." { ".$grid_width." ".$grid_height." }\n";
				$array_css[] = "\t.a-".$group->id." { width:".$group->adwidth."px; height:".$group->adheight."px; margin:".$group->admargin."px ".$group->admargin_right."px ".$group->admargin_bottom."px ".$group->admargin_left."px; }\n";
			}
...
/*-------------------------------------------------------------
 ...
 Purpose:   Add an advert to a single post or page
