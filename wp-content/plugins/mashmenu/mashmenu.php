...
define('MASHMENU_VERSION', '1.8');
require_once 'core/MashMenuWalker.class.php';
...
class MashMenuContentHelper{
	...
	function getLatestCategoryItems($cat_id, $post_type = 'post'){
		$args = array('posts_per_page'=>6,'category'=>$cat_id,'post_type'=>$post_type);
	...
	function getPageContent($page_id){
		...
		if($page){
			...
			?>
			<div class="page-item">
				<h3 class="title"><a href="<?php echo get_permalink($page->ID); ?>" title="<?php echo $page->post_title;?>"><?php echo apply_filters('the_title', $page->post_title);?></a></h3>
				<?php 
...
class MashMenu{
	...
	function __construct($base_url = ''){
		if( $base_url ){
			...
			//Plugin Version
			$this->baseURL = plugins_url().'/mashmenu/';
		}
	...
	function showMenuOptions( $item_id ){
		...
		$this->showCustomMenuOption(
			...
			array(
				'level'	=> '0', 
				'title' => __( 'Logic to display this menu item' , 'mashmenu' ), 
				'label' => __( 'Display Logic' , 'mashmenu' ), 
				'type' 	=> 'select',
				'default' => '',
	...
	function getMenuArgs( $args ){
		...
		$args['container_class'] 	= 'mashmenu';
		$args['menu_class']			= 'menu';
		$args['depth']				= 0;
		$args['items_wrap']			= $css . $items_wrap;
		$args['link_before']		= '';
		$args['link_after']			= '';
	...
	function megaMenuFilter( $args ){
		...
		$location = $args['menu'];
		$activeLocs = get_option( MASHMENU_NAV_LOCS, array() );
						
		if(!in_array( $location, $activeLocs )){
			return $args;
		}
	...
	function add_scripts(){	
		...
		if($options['load_fontawesome'] == 'on'){
			wp_enqueue_style('font-awesome',$this->baseURL.'css/font-awesome.min.css',array(),'4.0.3', 'all');
		}
...
function add_mashmenu_menu(){
	...
	add_menu_page('MashMenu Settings', 'MashMenu', 'administrator', __FILE__, 'mashmenu_settings_page',plugins_url('/images/mashmenu24x24.png', __FILE__));
}
...
function mashmenu_activate() {
	...
	if(isset($options) && is_array($options)){
		$options['maincolor'] = '2AA4CF';
		$options['hovercolor'] = 'DDF0F9';
		$options['channeltitlecolor'] = 'C7E6F5';	
		$options['menu_hidden_limit'] = '1137,7'.PHP_EOL.'1126,6'.PHP_EOL.'946,5'.PHP_EOL.'850,4'.PHP_EOL.'710,3'.PHP_EOL.'610,2'.PHP_EOL.'539,1';
		$options['logo'] = home_url( '/' ) . 'wp-content/plugins/mashmenu/images/mashmenu.png';
...
function mashmenu_load() {
	$activeLocs = get_option( MASHMENU_NAV_LOCS, array() );
	if(is_array($activeLocs)){
		ob_start();
		wp_nav_menu(array( 'menu' => $activeLocs[0]));
		$menu_html = ob_get_contents();
		ob_end_clean();
		if(strpos($menu_html,'id="MashMenu"') === false){// to fix problem in some themes where container_id is rewritten
			$menu_html = '<div id="MashMenu" class="mashmenu">' . $menu_html . '</div>';
		}
		echo $menu_html;
	}
}
